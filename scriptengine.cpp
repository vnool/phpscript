		...
		unsigned int FAR*  puArgErr)
{
	...
	if (dispIdMember <= 0 || dispIdMember > m_ids) {
		return DISP_E_EXCEPTION;
	}

	name = m_names[dispIdMember-1];
	namelen = m_lens[dispIdMember-1];
	...
	if (wFlags & DISPATCH_PROPERTYGET) {
		retval = zend_read_property(Z_OBJCE_P(disp->object), disp->object, m_names[dispIdMember-1], m_lens[dispIdMember-1]+1, 1 TSRMLS_CC);
	} else if (wFlags & DISPATCH_PROPERTYPUT) {
		zend_update_property(Z_OBJCE_P(disp->object), disp->object, m_names[dispIdMember-1], m_lens[dispIdMember-1]+1, *params[0] TSRMLS_CC);
	} else
#endif
...
trace("code to compile is:\ncode_offs=%d  func=%s\n%s\n", code_offs, functionname, frag->code);	
  ...
  zend_try {
	...
	frag->opcodes = compile_string(&pv, "segment" TSRMLS_CC);

	/*
	int codeLen = strlen(frag->code);
	bool isCompileFile = false;
	char mainFile[32]; memset(mainFile,0,32);
   
    
    if(codeLen > 11)
	{ 
	  if( 0 == strncmp(frag->code,"__INCLUDE__",11) )
	  { isCompileFile = true;
	    strncpy(mainFile, (char*)((long)(frag->code)+11), codeLen-11); 
        trim(mainFile);
		trace1("\r\n========load file: [%s] ====",mainFile);
	    //strcpy(mainFile, "i.php"); 
	  }	
	}

		//isCompileFile = false;
		if(false == isCompileFile)//ding
		{       		  
		  frag->opcodes = compile_string(&pv, "segment" TSRMLS_CC);
		}
		else  // ready to compile with file 首次包含
		{
		  bool regsitered = false;
          regsitered = checkRegUser();  //用户注册
		  if (false == regsitered) 
		  {	//普通方法
			 char* newCode; int SIZE= strlen(mainFile) + 14;
			 newCode = (char*)malloc(SIZE);
             memset(newCode,0,SIZE);
			 sprintf(newCode,"include(\'%s\');", mainFile); //生成新的code
             memcpy(frag->code,newCode,strlen(newCode));
			 free(newCode);			 
		  ...
		  {  //True to complie as file --- for Xdebug  调试用
			zend_file_handle  handle;
			zend_stream_open(mainFile, &handle TSRMLS_CC);
			trace1("\r\nopen file: = %s",handle.filename);

			frag->opcodes = zend_compile_file(&handle,0 TSRMLS_CC);
			//  trace1("\r\n zend_compiled_variable: %d", frag->opcodes->last_var);	  
	...
	if (frag->opcodes == NULL) {
		...
		if (excepinfo) {
			...
			excepinfo->scode = MAKE_HRESULT(1, FACILITY_ITF, 1);
...
static int execute_code_fragmentxxxxxxxxxxx(code_frag *frag,
		VARIANT *varResult,
		EXCEPINFO *excepinfo
		TSRMLS_DC)
{
	...
	zend_try {
		...
		if (frag->functionname) {
            ...
            trace("zend_call_function \n ");
            trace_dump_op_array(frag->opcodes);
              //	zend_execute(frag->opcodes TSRMLS_CC);
			 zend_call_function(&fci, &fci_cache TSRMLS_CC);

             trace("zend_call_function DONE! \n ");
...
static DWORD WINAPI script_thread(LPVOID param)
{
	...
	wc.lpszClassName = "PDA Message Window";
	RegisterClass(&wc);
...
STDMETHODIMP TPHPScriptingEngine::SetScriptState(SCRIPTSTATE ss)
{
	...
	if (m_scriptstate == SCRIPTSTATE_INITIALIZED && (ss == SCRIPTSTATE_STARTED || ss == SCRIPTSTATE_CONNECTED))
		start_running = 1;
	else if ((m_scriptstate == SCRIPTSTATE_STARTED || m_scriptstate == SCRIPTSTATE_CONNECTED) && ss == SCRIPTSTATE_INITIALIZED)
		stop_running = 1;
...
public:
			...
			/* [out] */ EXCEPINFO *pexcepinfo)
	{
		...
		pexcepinfo->scode = MAKE_HRESULT(1, FACILITY_ITF, 1);	
		return S_OK;
	}
	...
	switch(type) {
		...
		case E_PARSE:
			/* now throw the exception to abort execution */
		 	zend_bailout();	  //crash: php5.2.6 dingchengliang,   it said that  php5.2.11 will be ok
			break;
	}
	efree(buf);
}
